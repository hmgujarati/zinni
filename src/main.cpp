...
#if defined(NDEBUG)
#error "Zinni cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Zinni Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 1) return 240000 * COIN;
		
	int64_t nSubsidy;
	
	if( nHeight > 1 && nHeight <= 5000 ) {
	        nSubsidy = 1 * COIN;
	}else if( nHeight > 5000 && nHeight <= 30000 ) {
	        nSubsidy = 60 * COIN;
	} else {
		nSubsidy = 20 * COIN;
	}
